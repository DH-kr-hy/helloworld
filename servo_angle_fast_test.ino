...
int first_angle = 140; //각도 조절 변수, 초기값을 직진 값으로 설정->이런식으로 전역변수 선언하면 루프에서 영향 갈듯
...
void loop() {
  ...
  int servo_angle;
  for(servo_angle=40; servo_angle<=140; servo_angle += 2 ){
    myservo.write(servo_angle);
    delay(20); //0.5초간 대기
  }
  delay(1500);
  //직진상태에서부터 10씩 감소하며 왼쪽 최대각도까지 확인
  for(servo_angle=140; servo_angle>=40; servo_angle -= 2){
    myservo.write(servo_angle);
    delay(20); //0.5초간 대기
  }
  delay(1500);
