...
#define RIGHT_MAX_ANGLE 130 //서보 오른쪽 최대값
#define LEFT_MAX_ANGLE 50 // 서보 왼쪽 최대값
...
char LtoR(int angle){
  for(int servo_angle=50; servo_angle<=angle; servo_angle += 2){
    motor_servo.write(servo_angle);
    delay(70); //0.5초간 대기
  }
  return 'R';
...
char RtoL(int angle){
  for(int servo_angle=130; servo_angle>=angle; servo_angle -= 2){
    motor_servo.write(servo_angle);
    delay(70); //0.5초간 대기
  }
  return 'L';
...
void loop() {
    ...
    if (aver_sensor_value>200){//완전히 선이 정중앙에 있을 때 측정값 넣어서 바꾸기(320->400)
        error = aver_sensor_value - 200;//(320->400)
        ...
        if (servo_angle == LEFT_MAX_ANGLE)
            ...
            else if(present_position == 'C'){
              present_position = CtoL(servo_angle+output);
            }
            else{
              present_position = LtoL(servo_angle+output);
            }
            ...
            if(present_position == 'L'){
              ...
              present_position = CtoR(servo_angle-output);
            }
            else{
              present_position = RtoR(servo_angle-output);
            }
